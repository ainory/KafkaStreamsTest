...
Apache Kafka Streams를 사용하여 collectd 모니터링 데이터의 실시간 스트림 처리를 위한 포괄적인 데모 프로젝트입니다. 이 프로젝트는 확장 가능한 데이터 처리 파이프라인 구축을 위한 DSL(Domain Specific Language)과 Processor API 방식을 모두 보여줍니다.

## 🎯 개요

Kafka Streams Test 프로젝트는 collectd 모니터링 데이터를 실시간으로 처리하며, 시간 윈도우에 걸친 집계와 계산을 수행합니다. 이 프로젝트가 보여주는 것들:

- **실시간 데이터 처리:** 모니터링 메트릭(CPU, 메모리, 디스크 I/O)의 스트림 처리
- **윈도우 집계:** Tumbling 및 Hopping 윈도우를 사용한 시간 기반 그룹화
- **다중 처리 패턴:** 고수준 DSL과 저수준 Processor API 구현 모두
- **확장 가능한 아키텍처:** 상태 관리를 통한 수평적 확장 가능한 스트림 처리
...
2. **Kafka Plugin**이 JSON 데이터를 `COLLECTD_DATA` 토픽으로 전송
...
- 고수준 함수형 프로그래밍 스타일
- 자동 윈도우잉과 집계
- 사용자 정의 serdes를 사용한 타입 안전 연산
- 내장된 장애 허용성

#### Processor API
- 처리 로직에 대한 저수준 제어
- 사용자 정의 상태 관리
- 유연한 스케줄링과 punctuation
- 세밀한 오류 처리
...
- **📊 실시간 모니터링:** 도착하는 collectd 메트릭을 실시간 처리
- **⏱️ 윈도우 집계:** 주기적 요약을 위한 60초 Tumbling 윈도우
- **📈 통계 연산:** 최솟값, 최댓값, 평균, 합계 계산
- **🔄 다중 API:** DSL과 Processor API 구현 모두
- **💾 상태 관리:** 영구 및 인메모리 state store
- **⚡ 고성능:** 처리량과 낮은 지연 시간에 최적화

### 지원하는 메트릭

- **CPU 사용률:** 코어별 및 전체 CPU 사용률
- **메모리 통계:** 메모리 사용 패턴과 추세
- **디스크 I/O:** 읽기/쓰기 연산과 처리량
- **로드 평균:** 시스템 로드 메트릭 (1분, 5분, 15분)
- **네트워크 활동:** 인터페이스 통계와 대역폭
...
- **[API 문서](docs/API_DOCUMENTATION_KR.md)** - 예제와 함께 하는 포괄적인 API 레퍼런스
- **[사용 가이드](docs/USAGE_GUIDE_KR.md)** - 상세한 설정, 구성, 사용 지침
...
| 컴포넌트 | 목적 | 문서 |
|----------|------|------|
| `DslTestMain` | DSL 기반 스트림 처리 | [API 문서](docs/API_DOCUMENTATION_KR.md#dsltestmain) |
| `ProcessorApiTestMain` | Processor API 구현 | [API 문서](docs/API_DOCUMENTATION_KR.md#processorapitestmain) |
| `CollectdKafkaVO` | 입력 데이터 구조 | [API 문서](docs/API_DOCUMENTATION_KR.md#collectdkafkavo) |
| `HostMetricVO` | 집계된 출력 형식 | [API 문서](docs/API_DOCUMENTATION_KR.md#hostmetricvo) |
| `CustomSerdes` | 직렬화 유틸리티 | [API 문서](docs/API_DOCUMENTATION_KR.md#customserdes) |
| `JsonUtil` | JSON 처리 유틸리티 | [API 문서](docs/API_DOCUMENTATION_KR.md#jsonutil) |
...
│   │   └── CollectdTimestampExtractor.java # 사용자 정의 타임스탬프 로직
│   └── util/                               # 유틸리티 클래스
│       └── JsonUtil.java                  # JSON 처리
├── docs/                                   # 문서
│   ├── API_DOCUMENTATION_KR.md            # 완전한 API 레퍼런스
│   └── USAGE_GUIDE_KR.md                  # 설정 및 사용 가이드
├── pom.xml                                 # Maven 설정
├── README_KR.md                            # 이 파일
└── LICENSE                                 # 라이선스 정보
```
...
| 토픽 | 목적 | 파티션 | 보존 기간 |
|------|------|--------|----------|
| `COLLECTD_DATA` | collectd 입력 메트릭 | 3 | 24시간 |
...
- **지연 시간:** 종단 간 50-200ms
- **메모리 사용량:** 1-4GB 힙 권장
- **State Store:** 100MB-1GB 일반적 크기

### 최적화 팁

1. **병렬 처리 증가:** 더 많은 파티션과 스트림 스레드 추가
2. **JVM 튜닝:** 더 나은 일시 정지 시간을 위해 G1GC 사용
3. **버퍼링 설정:** 워크로드에 맞게 캐시 크기 조정
4. **지연 모니터링:** 컨슈머 지연과 처리 지연 감시
...
- **Java 8+** 기능과 패턴
- 적절한 곳에서 **함수형 프로그래밍**
- 우아한 성능 저하를 가진 **포괄적인 오류 처리**
- 모니터링과 디버깅을 위한 **상세한 로깅**
...
1. **디버그 로깅 활성화** - 상세한 처리 정보를 위해
2. **컨슈머 지연 모니터링** - Kafka 도구 사용
3. **데이터 형식 확인** - 콘솔 컨슈머로 메시지 형식 검사
4. **State Store 확인** - 크기와 내용 모니터링
5. **네트워크 연결성 검증** - 브로커 연결성과 DNS 해결 확인
...
이 프로젝트는 [LICENSE](LICENSE) 파일에 명시된 조건 하에 라이선스가 부여됩니다.
...
- **이슈:** GitHub 이슈 열기
- **API 레퍼런스:** [API_DOCUMENTATION_KR.md](docs/API_DOCUMENTATION_KR.md) 참조
- **설정 도움:** [USAGE_GUIDE_KR.md](docs/USAGE_GUIDE_KR.md) 참조
